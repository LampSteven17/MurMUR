...
func renderOperatorReadme(u User, dir string, hasBinary bool) string {
	...
	fmt.Fprintf(&b, "## Using the TUI\n\n")
	fmt.Fprintf(&b, "Launch `./murmur tui`; the first keypress dismisses the splash. The top\n")
	fmt.Fprintf(&b, "bar shows only the tabs your role can use, and list views show only the\n")
	fmt.Fprintf(&b, "guests you deployed.\n\n")
	fmt.Fprintf(&b, "Keys:\n")
	fmt.Fprintf(&b, "- `1`–`5` — views: overview · VMs · LXCs · nodes · templates (read-only)\n")
	fmt.Fprintf(&b, "- `a` apps · `d` deploy · `t` teardown · `p` patch — actions\n")
	fmt.Fprintf(&b, "  (admins also get `u` host-update and `x` users)\n")
	fmt.Fprintf(&b, "- `r` refresh · `q` quit\n\n")
	fmt.Fprintf(&b, "Common tasks:\n")
	fmt.Fprintf(&b, "- **Deploy an app** — `a`, then ↑/↓ to move, space to select, enter to\n")
	fmt.Fprintf(&b, "  confirm; answer any secret prompts. The guest lands in your pool, tagged\n")
	fmt.Fprintf(&b, "  to you. You can only deploy the apps your role allows.\n")
	fmt.Fprintf(&b, "- **Deploy a one-off VM** — `d`, fill the form (image, flavor, name), confirm.\n")
	fmt.Fprintf(&b, "- **Update an app's containers** — `p`, pick a running app; murmur SSHes in\n")
	fmt.Fprintf(&b, "  and runs its `update:` command. (This is app-level, not host upgrades.)\n")
	fmt.Fprintf(&b, "- **Tear down** — `t`, multi-select your guests, confirm (stop + destroy).\n\n")
	fmt.Fprintf(&b, "## Notes\n\n")
